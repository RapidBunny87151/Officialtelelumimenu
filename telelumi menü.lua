-- ============================================================================
-- LUMII PREMIUM HUB - ULTRA MODERNES GLASSMORPHISM DESIGN 2.0
-- Xeno-kompatibel mit Premium UI-Design
-- Features: Glassmorphism, Neon-Glows, Backdrop Blur, Schatten, Animationen
-- Inspiriert von: iOS 17, Windows 11, Fluent Design, Modern Web Apps
-- ============================================================================

-- Xeno Compatibility Layer - ERWEITERT
if not gethui then getgenv().gethui = function() return game:GetService("CoreGui") end end
if not getcustomasset then getgenv().getcustomasset = function(path) return "rbxasset://"..path end end
if not isfolder then getgenv().isfolder = function() return false end end
if not makefolder then getgenv().makefolder = function() end end
if not writefile then getgenv().writefile = function() end end
if not readfile then getgenv().readfile = function() return nil end end
if not isfile then getgenv().isfile = function() return false end end
if not setfpscap then getgenv().setfpscap = function() end end
if not getconnections then getgenv().getconnections = function() return {} end end
if not setclipboard then getgenv().setclipboard = function(text) warn("setclipboard wird nicht unterstützt") end end
...
-- KRITISCHE FEHLENDE FUNKTIONEN HINZUFÜGEN
if not getgenv().SIRIUS_LOAD_UI_SAFE then
    getgenv().SIRIUS_LOAD_UI_SAFE = function(assetId, name, parent)
        ...
        if success and UI then
            ...
            warn("[SIRIUS] Konnte UI nicht von Asset "..assetId.." laden, erstelle Ultra-Premium Fallback UI")
            ...
            -- Erstelle minimale UI-Struktur
            Instance.new("Frame", ScreenGui).Name = "Character"
            Instance.new("Frame", ScreenGui).Name = "Scripts"
            ...
            Instance.new("Frame", ScreenGui).Name = "Notifications"
            Instance.new("Frame", ScreenGui).Name = "Toasts"
            Instance.new("Frame", ScreenGui).Name = "CustomScriptPrompt"
            Instance.new("Frame", ScreenGui).Name = "SecurityPrompt"
            Instance.new("Frame", ScreenGui).Name = "Disconnected"
            Instance.new("Frame", ScreenGui).Name = "GameDetection"
            Instance.new("Frame", ScreenGui).Name = "ModeratorDetectionPrompt"
            Instance.new("Frame", ScreenGui).Name = "ScriptSearch"
            Instance.new("Frame", ScreenGui).Name = "Starlight"
            
            warn("[SIRIUS] ✨ Ultra-Premium Fallback UI erstellt ✨")
            return ScreenGui
        end
    end
end

if not getgenv().SIRIUS_TELEPORT_FIX then
    getgenv().SIRIUS_TELEPORT_FIX = function(player, localPlayer, queueNotification)
        pcall(function()
            ...
            if character and targetCharacter then
                ...
                if humanoidRootPart and targetRootPart then
                    ...
                    if queueNotification then
                        queueNotification("Teleportiert", "Zu "..player.DisplayName.." teleportiert", 4335479121)
                    end
                end
            end
        end)
...
print("✓ Xeno Compatibility aktiviert - Alle Funktionen geladen")
...
local siriusValues = {
	...
	actions = {
		...
		},
		{
			...
			color = Color3.fromRGB(20, 150, 200),
			enabled = false,
		...
		},
		{
			...
			color = Color3.fromRGB(60, 120, 230),
			enabled = false,
			rotateWhileEnabled = true,
			disableAfter = 2,
			callback = function()
		...
		},
		{
			...
			color = Color3.fromRGB(80, 60, 220),
			enabled = false,
			rotateWhileEnabled = false,
			callback = function() end,
		},
		{
			...
			color = Color3.fromRGB(40, 140, 200),
			enabled = false,
			rotateWhileEnabled = true,
			callback = function(value)
				local character = localPlayer.Character
		...
		},
		{
			...
			color = Color3.fromRGB(100, 160, 240),
			enabled = false,
			rotateWhileEnabled = false,
			callback = function(value)
		...
		},
		{
			...
			color = Color3.fromRGB(50, 80, 180),
			enabled = false,
			rotateWhileEnabled = false,
			callback = function(value)
		...
		},
		{
			...
			color = Color3.fromRGB(70, 130, 210),
			enabled = false,
			rotateWhileEnabled = false,
			callback = function(value)
				if value then
	...
	sliders = {
		{
			name = "player speed",
			color = Color3.fromRGB(40, 130, 220),
			values = {0, 300},
			default = 16,
			value = 16,
			active = false,
...
local siriusSettings = {
	...
	},
	{
		...
		color = Color3.new(0.15, 0.6, 0.85),
		minimumLicense = 'Free',
		categorySettings = {
			{
	...
	},
	{
		...
		color = Color3.new(0.25, 0.45, 0.75),
		minimumLicense = 'Free',
		categorySettings = {
			{
	...
	},
	{
		...
		color = Color3.new(0.35, 0.65, 0.95),
		minimumLicense = 'Free',
		categorySettings = {
			{
				name = 'Log Messages',
...
local UI = getgenv().SIRIUS_LOAD_UI_SAFE(siriusValues.interfaceAsset, siriusValues.siriusName, guiParent)
...
local function teleportTo(player)
    return getgenv().SIRIUS_TELEPORT_FIX(player, localPlayer, queueNotification)
end
...
homeContainer.Interactions.Discord.Interact.MouseButton1Click:Connect(function()
	if setclipboard then 
		originalSetClipboard("https://discord.gg/bcEXr9RXFu")
